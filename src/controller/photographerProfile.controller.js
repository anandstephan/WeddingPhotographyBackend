...
const createFolderWithPhotos = asyncHandler(async (req, res) => {
    ...
    if (photographer.portfolio.some(folder => folder.folderName === folderName)) {
    ...
    for (const file of files) {
        const fileKey = `portfolio/${user.mobile}/${Date.now()}_${file.originalname}`;
    ...
    res.status(201).json({ message: "Folder created with photos successfully", folderName, photos: uploadedPhotos });
});
...
const removeImagesFromFolder = asyncHandler(async (req, res) => {
    ...
    const folder = photographer.portfolio.find(folder => folder.folderName === folderName);
    ...
    folder.photos = folder.photos.filter(photo => !photosToRemove.includes(photo));
    await photographer.save();

    res.status(200).json({ message: "Photos removed successfully", remainingPhotos: folder.photos });
});
...
const deleteFolder = asyncHandler(async (req, res) => {
    ...
    const folderIndex = photographer.portfolio.findIndex(folder => folder.folderName === folderName);
...
const addPhotosToFolder = asyncHandler(async (req, res) => {
    ...
    res.status(200).json({ message: "Photos added successfully", photos: uploadedPhotos });
});
...
export { createProfile, updateProfile, getProfilebyId, deleteProfile, getPhotographerProfile, createFolderWithPhotos, removeImagesFromFolder, deleteFolder, addPhotosToFolder, getPhotographersList };
